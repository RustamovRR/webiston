...
**Kontekstual qidiruv** (_contextual retrieval_) g'oyasining mohiyati shuki, har bir bo'lakni tegishli kontekst bilan boyitish orqali uni qidirib topishni osonlashtirish mumkin. Oddiy usul — bo'lakni teglar va kalit so'zlar kabi metadata bilan to'ldirish. Elektron tijoratda mahsulotni uning tavsifi va sharhlari bilan boyitish mumkin. Rasm va videolarni esa sarlavha yoki izoh orqali qidirish mumkin.

Metadata bo'lakdan avtomatik ajratib olingan obyektlar (_entities_) — shaxs ismlari, kompaniya nomlari, mahsulot kodlari kabi nomli birliklarni — ham o'z ichiga olishi mumkin. Agar hujjatingizda `EADDRNOTAVAIL (99)` kabi maxsus xatolik kodi bo'lsa, uni hujjat metadatasiga qo'shish tizimga ushbu kalit so'z bo'yicha qidirishga imkon beradi — hatto hujjat _embedding_'ga aylantirilgan bo'lsa ham.

Har bir bo'lakni u javob bera oladigan **savollar** bilan ham boyitish mumkin. Mijozlarga xizmat ko'rsatishda har bir maqolani tegishli savollar bilan to'ldirish mumkin. Masalan, parolni tiklash haqidagi maqolaga "Parolni qanday tiklash mumkin?", "Parolimni unutdim", "Tizimga kira olmayapman" yoki "Hisobimni topa olmayapman" kabi so'rovlarni qo'shish mumkin.[^9]

Agar hujjat bir nechta bo'lakka bo'lingan bo'lsa, ba'zi bo'laklar qidiruvchiga bo'lakning mazmunini tushunishga yetarli kontekst bermaydi. Buning oldini olish uchun har bir bo'lakni asl hujjatdan olingan kontekst — masalan, hujjatning sarlavhasi va qisqacha mazmuni — bilan boyitish mumkin. Anthropic _SI_ modellaridan foydalanib, bo'lakning asl hujjat bilan aloqasini tushuntiruvchi qisqa kontekst (odatda 50-100 token) yaratgan. Anthropic bu maqsadda quyidagi promptdan foydalangan ([Anthropic, 2024](https://www.anthropic.com/engineering/contextual-retrieval)):
...
`Quyida ushbu bo'lakni butun hujjat ichida joylashtirmoqchi bo'lgan qism:`

<chunk>
{{CHUNK_CONTENT}}
</chunk>

`Iltimos, bo'lakni qidiruv samaradorligini oshirish maqsadida 
butun hujjat ichidagi o'rnini ko'rsatuvchi qisqa va lo'nda 
kontekst bering. Faqat shu qisqa kontekstni yozing, boshqa hech narsa yo'q.`
```

Har bir bo'lak uchun yaratilgan kontekst bo'lak boshiga qo'shiladi, so'ngra boyitilgan bo'lak qidiruv algoritmi tomonidan indekslanadi. 6-5-rasmda Anthropic qo'llagan jarayon tasvirlangan.

![6-5-rasm. Anthropic har bir bo'lakni qisqa kontekst bilan boyitadi — bu qidiruvchiga berilgan so'rov bo'yicha tegishli bo'laklarni topishni osonlashtiradi.](/ai-engineering/6-chapter/6.5-figure.png)
<div className='text-center text-sm italic'>6-5-rasm. Anthropic har bir bo'lakni qisqa kontekst bilan boyitadi — bu qidiruvchiga berilgan so'rov bo'yicha tegishli bo'laklarni topishni osonlashtiradi. "Introducing Contextual Retrieval" (Anthropic, 2024) maqolasidan olindi.</div>

> #### Qidiruv yechimlarini baholash
> 
> Qidiruv yechimini tanlashda e'tiborga olish kerak bo'lgan asosiy omillar:
> 
> - Qanday qidiruv mexanizmlarini qo'llab-quvvatlaydi? Gibrid qidiruvni qo'llab-quvvatlaydimi?
> - Agar vektorli ma'lumotlar bazasi bo'lsa, qaysi _embedding_ modellari va vektorli qidiruv algoritmlarini qo'llab-quvvatlaydi?
> - Ma'lumotlarni saqlash va so'rov trafigi jihatidan qanchalik kengayuvchan? Sizning trafik andozalaringizga mos keladimi?
> - Ma'lumotlarni indekslash qancha vaqt oladi? Bir vaqtning o'zida qancha ma'lumotni (qo'shish/o'chirish) qayta ishlash mumkin?
> - Turli qidiruv algoritmlari uchun so'rov kechikishi qanday?
> - Agar boshqariladigan yechim bo'lsa, narxlash tuzilmasi qanday? Hujjat/vektor hajmiga yoki so'rov hajmiga qarab hisoblanadimi?
> 
> Bu ro'yxatda kirish nazorati, muvofiqlik, ma'lumotlar tekisligi va boshqaruv tekisligini ajratish kabi korporativ yechimlar bilan bog'liq funksionallik ko'rsatkichlari kiritilmagan.

## Matndan tashqari _RAG_

Yuqorida matn hujjatlaridan foydalanadigan _RAG_ tizimlari muhokama qilindi. Biroq tashqi ma'lumot manbalari ko'p modalli (_multimodal_) yoki jadval ko'rinishida ham bo'lishi mumkin.

### Ko'p modalli _RAG_

Agar generatoringiz ko'p modalli bo'lsa, kontekst nafaqat matn hujjatlari, balki tashqi manbalardan olingan rasmlar, videolar, audio va boshqalar bilan ham boyitilishi mumkin. Qisqalik uchun misollarda faqat rasmlardan foydalanamiz — ularni istalgan boshqa modallik bilan almashtirish mumkin. Berilgan so'rov asosida qidiruvchi unga aloqador matnlar va rasmlarni birga olib keladi. Masalan, "Pixar'ning 'Up' multfilmidagi uyning rangi qanday?" degan so'rovga javob berish uchun qidiruvchi 6-6-rasmda ko'rsatilganidek, "Up" dagi uy rasmini olib kelishi mumkin.

![6-6-rasm. Ko'p modalli RAG so'rovni matn va rasmlar bilan birga boyitishi mumkin.](/ai-engineering/6-chapter/6.6-figure.png)
<div className='text-center text-sm italic'>6-6-rasm. Ko'p modalli _RAG_ so'rovni matn va rasmlar bilan birga boyitishi mumkin. (*Mualliflik huquqi sababli "Up" filmidagi haqiqiy rasm ishlatilmagan.)</div>

Agar rasmlarda metadata — sarlavha, teglar, izohlar — mavjud bo'lsa, ularni metadata orqali qidirib topish mumkin. Masalan, rasm izohi so'rovga aloqador deb topilsa, o'sha rasm qidirib olinadi.

Rasmlarni mazmuniga qarab qidirish uchun esa rasmlarni so'rovlar bilan taqqoslash imkoniyati kerak. Agar so'rovlar matn ko'rinishida bo'lsa, matn va rasmlar uchun ham _embedding_ yarata oladigan ko'p modalli _embedding_ modeli zarur. Masalan, `CLIP` ([Radford va boshqalar, 2021](https://arxiv.org/abs/2103.00020)) ko'p modalli _embedding_ modeli sifatida ishlatilsa, qidiruvchi quyidagicha ishlaydi:

1. Barcha ma'lumotlar — matnlar va rasmlar — uchun `CLIP` _embedding_'lari yaratiladi va vektorli ma'lumotlar bazasiga saqlanadi.
2. Berilgan so'rov uchun uning `CLIP` _embedding_'i yaratiladi.
3. Vektorli bazada so'rov _embedding_'iga yaqin barcha rasm va matnlar qidirib topiladi.

### Jadval ma'lumotlari bilan _RAG_

Ko'pgina ilovalar faqat matn va rasmlar kabi strukturalashmagan ma'lumotlar bilan emas, balki jadval ma'lumotlari bilan ham ishlaydi. Ko'plab so'rovlarga javob berish uchun ma'lumotlar jadvallaridan ma'lumot olish kerak bo'ladi. Jadval ma'lumotlari yordamida kontekstni boyitish jarayoni klassik _RAG_ jarayonidan sezilarli darajada farq qiladi.

Tasavvur qiling, siz mushuk modasi bilan ixtisoslashgan "Kitty Vogue" nomli elektron tijorat saytida ishlayapsiz. Bu do'konda 6-3-jadvalda ko'rsatilganidek, _Sales_ nomli buyurtmalar jadvali mavjud.

| Buyurtma ID | Vaqt | Mahsulot ID | Mahsulot | Birlik narxi ($) | Miqdor | Jami |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
<div className='text-center text-sm italic'>6-3-jadval. "Kitty Vogue" tasavvuriy elektron tijorat saytining buyurtmalar jadvali — Sales.</div>

"So'nggi 7 kunda Fruity Fedora'dan nechta dona sotilgan?" degan savolga javob berish uchun tizim ushbu jadvaldan Fruity Fedora bilan bog'liq barcha buyurtmalarni so'rab, miqdorlarni yig'ishi kerak. Jadval _SQL_ orqali so'raladi deb faraz qilsak, _SQL_ so'rovi quyidagicha ko'rinadi:
...
Ish jarayoni quyidagicha — 6.7-rasmda ko'rsatilgan. Ushbu jarayonni ishga tushirish uchun tizim _SQL_ so'rovini yaratish va bajarish qobiliyatiga ega bo'lishi shart:

1. **Matndan _SQL_'ga** (_Text-to-SQL_): foydalanuvchi so'rovi va jadval sxemalari asosida kerakli _SQL_ so'rovi aniqlanadi. Matndan _SQL_'ga o'tkazish — [2-bobda](/books/ai-engineering/2-understanding-foundation-models) muhokama qilingan semantik tahlilning bir ko'rinishi.
2. **_SQL_ bajarish**: _SQL_ so'rovi ishga tushiriladi.
3. **Generatsiya**: _SQL_ natijasi va asl foydalanuvchi so'rovi asosida javob yaratiladi.

![6-7-rasm. Jadval ma'lumotlari bilan kontekstni boyituvchi RAG tizimi.](/ai-engineering/6-chapter/6.7-figure.png)
<div className='text-center text-sm italic'>6-7-rasm. Jadval ma'lumotlari bilan kontekstni boyituvchi _RAG_ tizimi.</div>

Matndan _SQL_'ga o'tkazish bosqichida, agar mavjud jadvallar sxemalari model kontekstiga sig'masa, qaysi jadvallardan foydalanish kerakligini oldindan aniqlash uchun oraliq qadam kerak bo'lishi mumkin. Bu vazifani yakuniy javobni yaratuvchi generator yoki ixtisoslashgan matndan _SQL_'ga o'tkazuvchi model bajarishi mumkin.

Ushbu bo'limda qidiruvchilar va _SQL_ ishga tushiruvchilari (_executors_) kabi vositalar modelga ko'proq so'rovlarni qayta ishlash va yuqori sifatli javoblar yaratish imkonini berishini ko'rdik. Modelga yanada ko'proq vositalar berish uning imkoniyatlarini yanada kengaytiradimi? Vositalardan foydalanish — keyingi bo'limda muhokama qilinadigan agent andozasining asosiy xususiyatlaridan biridir.
