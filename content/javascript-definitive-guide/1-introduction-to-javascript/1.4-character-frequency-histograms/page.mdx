---
title: "Misol: Belgilar chastotasi gistogrammalari"
description: "Belgilar chastotasi gistogrammasi: Node'da standart kiritishdan matn o'qish, Map va sinflar bilan hisoblash, foizlarga aylantirish hamda ASCII grafik ko'rinishida chiqarish."
keywords: JavaScript, Node.js, Map, sinf, class, gistogramma, histogram, matn tahlili, text analysis, chastota, frequency, ASCII grafika, ASCII graphics, terminal, misol, example, belgilar, characters
author: Webiston.uz
---
...
class DefaultMap extends Map {
  ...
  get(key) {
    if (this.has(key)) {                            // Agar kalit map'da mavjud bo'lsa,
...
class Histogram {
  ...
  toString() {
    ...
    entries.sort((a, b) => {                        // Saralash tartibini aniqlaydigan funksiya.
      if (a[1] === b[1]) {                          // Agar hisoblar bir xil bo'lsa
        return a[0] < b[0] ? -1 : 1                 // alifbo bo'yicha saralash
      } else {
        return b[1] - a[1]                          // Aks holda, eng katta sanoq bo'yicha saralash.
      }
    })
