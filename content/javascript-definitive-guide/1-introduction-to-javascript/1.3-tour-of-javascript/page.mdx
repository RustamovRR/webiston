...
let book = {
  // Obyektlar jingalak qavslar `{}` ichiga olinadi.
...
let points = [
  // 2 ta elementdan iborat massiv.
  { x: 0, y: 0 }, // Har bir element - obyekt.
  { x: 1, y: 1 },
]
let data = {
  // 2 ta xossaga ega obyekt
  trial1: [
    [1, 2],
    [3, 4],
  ], // Har bir xossaning qiymati — massiv.
  trial1: [
    [2, 3],
    [4, 5],
  ], // Bu massivlarning elementlari ham massivlar.
}
```
...
Bu `// =>` ko‘rinishidagi izohlar, shuningdek _tasdi (assertion)_ vazifasini ham bajaradi. Men kodni sinovdan o'tkazadigan va izohda ko'rsatilgan qiymatni hosil qilishini tekshiradigan maxsus vosita yozganman. Umid qilamanki, bu kitobdagi xatolarni kamaytirishga yordam beradi.
...
;(false ===
  (x > y)(
    // => true: `false` `false`ga teng

    // Mantiqiy operatorlar mantiqiy (boolean) qiymatlarni birlashtiradi yoki inversiya qiladi
    x === 2,
  ) &&
  (y === 3)(
    // => true: ikkala taqqoslash ham rost. `&&` — bu mantiqiy VA (AND).
    x > 3,
  )) ||
  y < 3 // => false: ikkala taqqoslash ham yolg‘on. `||` — bu mantiqiy YOKI (OR)
...
Siz yuqorida o'zgaruvchilarni e'lon qilish va tayinlash ko‘rsatmalarini ko'rdingiz. Ko'rsatmalarning yana bir katta toifasi - bu shartli ko'rsatmalar va sikllar kabi _boshqaruv tuzilmalaridir (control structures)_. Siz ularning misollarini funksiyalarni ko'rib chiqqanimizdan so'ng ko'rasiz.
...
function plus1(x) {
  // "plus1" nomli, "x" parametrli funksiyani aniqlash
...
let square = function (x) {
  // Funksiyalar qiymatlardir va o'zgaruvchilarga tayinlanishi mumkin
...
```js
// Funksiyalar obyektlarning xossalariga tayinlanganda, biz ularni
// "metodlar" deb ataymiz. Barcha JavaScript obyektlari (jumladan massivlar ham) metodlarga ega:
...
points.dist = function () {
  // Nuqtalar orasidagi masofani hisoblaydigan metodni aniqlash
...
function abs(x) {
  // Absolyut qiymatni hisoblaydigan funksiya
  if (x >= 0) {
    // `if` ko‘rsatmasi...
    return x // agar taqqoslash rost bo'lsa, shu kodni bajaradi
  } // `if` blokining oxiri
  else {
    // Ixtiyoriy `else` bloki, agar taqqoslash
...
function sum(array) {
  ...
  for (let x of array) {
    // Massiv bo'ylab sikl, har bir elementni x ga tayinlaydi
...
function factorial(n) {
  ...
  while (n > 1) {
    // `()` ichidagi ifoda rost bo'lsa, `{}` ichidagi ko'rsatmalarni takrorlaydi.
...
function factorial2(n) {
  // Boshqa sikl yordamida yozilgan versiya
  let i,
...
```js
class Point {
  // Qoida bo'yicha, klass nomlari bosh harf bilan yoziladi.
  constructor(x, y) {
    // Yangi nusxalarni initsializatsiya qilish, ya'ni ishga tushirish, uchun konstruktor funksiyasi
  ...
  distance() {
    // Nuqtadan koordinata boshigacha masofani hisoblaydigan metod.
    return Math.sqrt(
      // x² + y² ning kvadrat ildizini qaytarish.
