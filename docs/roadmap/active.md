...
| ToC rail indicator | ✅ | `border-l-2` on a `rounded-md` link followed the radius and drew a detached **arc** — it read as a stray bracket. Now the standard docs pattern: `ul` owns a continuous `border-l`, rows sit on it via `-ml-px border-l-2` with no radius, indentation via padding so the line never breaks |
