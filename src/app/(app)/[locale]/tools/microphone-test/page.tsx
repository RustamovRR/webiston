...
import { MicFaq, MicrophoneTest } from "@/modules/tools/MicrophoneTest"
import {
  applicationSchema,
  generateBreadcrumbSchema,
  generateFAQSchema,
  getMicrophoneTestMetadata
} from "@/modules/tools/MicrophoneTest/seo"
...
export async function generateMetadata({
  ...
  return withLocale(
    getMicrophoneTestMetadata(locale),
    locale,
...
export default async function MicrophoneTestPage({
  ...
  // The FAQ schema reads the same messages `MicFaq` renders, so the structured
  ...
  return (
    <>
      ...
      {/* A Server Component sibling of the client island: the answers are in
          ...
          actually ask. */}
      <MicFaq locale={locale} />
      <RelatedTools locale={locale} href="/tools/microphone-test" />
    </>
  )
}
