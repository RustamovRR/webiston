...
import {
  ...
  LoremIpsum,
  LoremReference
} from "@/modules/tools/LoremIpsum"
import {
  applicationSchema,
  generateBreadcrumbSchema,
  generateFAQSchema,
  getLoremIpsumMetadata
} from "@/modules/tools/LoremIpsum/seo"
...
export async function generateMetadata({
  ...
  return withLocale(getLoremIpsumMetadata(locale), locale, "/tools/lorem-ipsum")
}
...
export default async function LoremIpsumPage({
  ...
  // The FAQ schema reads the same messages `LoremFaq` renders, so the
  // structured data can never describe a page that does not exist.
  const tFaq = await getTranslations({
    locale,
    namespace: "LoremIpsumPage.faq"
  })
  ...
  return (
    <>
      ...
      <LoremReference locale={locale} />
      <LoremFaq locale={locale} />
      <RelatedTools locale={locale} href="/tools/lorem-ipsum" />
    </>
  )
}
