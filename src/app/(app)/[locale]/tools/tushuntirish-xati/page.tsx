...
import {
  ...
  generateFAQSchema,
  getTushuntirishMetadata
} from "@/modules/tools/Documents/templates/tushuntirish/seo"

// This document's namespace plus the shell's, and the shared `Common`.
const CLIENT_NAMESPACES = ["TushuntirishPage", "DocumentsShared", "Common"]
...
export async function generateMetadata({
  ...
  return withLocale(
    getTushuntirishMetadata(locale),
    locale,
    "/tools/tushuntirish-xati"
  )
}

export default async function TushuntirishPage({
  params
}: {
  params: Promise<{ locale: string }>
}) {
  ...
  const tFaq = await getTranslations({
    locale,
    namespace: "TushuntirishPage.faq"
  })
  ...
  return (
    <>
      ...
      <LocaleMessages locale={locale} namespaces={CLIENT_NAMESPACES}>
        <TushuntirishTool>
          <TemplateSwitcher locale={locale} current="tushuntirish" />
        </TushuntirishTool>
      </LocaleMessages>
      <DocumentFaq
        locale={locale}
        namespace="TushuntirishPage"
        keys={TUSHUNTIRISH_TEMPLATE.faqKeys}
      />
      <RelatedTools locale={locale} href="/tools/tushuntirish-xati" />
    </>
  )
}
