...
import { UrlEncoder, UrlFaq, UrlReference } from "@/modules/tools/UrlEncoder"
import {
  applicationSchema,
  generateBreadcrumbSchema,
  generateFAQSchema,
  getUrlEncoderMetadata
} from "@/modules/tools/UrlEncoder/seo"
...
export async function generateMetadata({
  ...
  return withLocale(getUrlEncoderMetadata(locale), locale, "/tools/url-encoder")
}
...
export default async function UrlEncoderPage({
  ...
  // The FAQ schema reads the same messages `UrlFaq` renders, so the structured
  // data can never describe a page that does not exist.
  const tFaq = await getTranslations({
    locale,
    namespace: "UrlEncoderPage.faq"
  })
  ...
  return (
    <>
      ...
      <UrlReference locale={locale} />
      <UrlFaq locale={locale} />
      <RelatedTools locale={locale} href="/tools/url-encoder" />
    </>
  )
}
