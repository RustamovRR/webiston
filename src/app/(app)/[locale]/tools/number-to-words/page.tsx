...
import { NumberToWords, NumberToWordsFaq } from "@/modules/tools/NumberToWords"
import {
  applicationSchema,
  generateBreadcrumbSchema,
  generateFAQSchema,
  getNumberToWordsMetadata
} from "@/modules/tools/NumberToWords/seo"

// Only this tool's namespace reaches the client, plus the shared `Common`.
const TOOL_NAMESPACE = "NumberToWordsPage"
...
export async function generateMetadata({
  ...
  return withLocale(
    getNumberToWordsMetadata(locale),
    locale,
    "/tools/number-to-words"
  )
}

export default async function NumberToWordsPage({
  params
}: {
  params: Promise<{ locale: string }>
}) {
  ...
  // The FAQ schema reads the same messages `NumberToWordsFaq` renders, so the
  ...
  return (
    <>
      ...
      <NumberToWordsFaq locale={locale} />
      <RelatedTools locale={locale} href="/tools/number-to-words" />
    </>
  )
}
