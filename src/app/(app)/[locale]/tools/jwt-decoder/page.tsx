...
// reference manifest.
import { JwtDecoder, JwtFaq, JwtReference } from "@/modules/tools/JwtDecoder"
import {
  applicationSchema,
  generateBreadcrumbSchema,
  generateFAQSchema,
  getJwtDecoderMetadata
} from "@/modules/tools/JwtDecoder/seo"

// Only this tool's namespace reaches the client, plus the shared
// `Common` used by ToolHeader. See LocaleMessages.
const TOOL_NAMESPACE = "JwtDecoderPage"
...
export async function generateMetadata({
  ...
  return withLocale(getJwtDecoderMetadata(locale), locale, "/tools/jwt-decoder")
}
...
export default async function JwtDecoderPage({
  ...
  // The FAQ schema reads the same messages `JwtFaq` renders, so the structured
  ...
  return (
    <>
      ...
      <JwtReference locale={locale} />
      <JwtFaq locale={locale} />
      <RelatedTools locale={locale} href="/tools/jwt-decoder" />
    </>
  )
}
