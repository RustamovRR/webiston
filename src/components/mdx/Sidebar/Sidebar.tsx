...
const Sidebar = memo(({ tutorialId, navigationItems, onLinkClick }: SidebarProps) => {
  ...
  const findItemByPath = useCallback((items: TutorialNavigation[], targetPath: string): TutorialNavigation | null => {
    for (const item of items) {
  ...
  const [openAccordionItems, setOpenAccordionItems] = useState<string[]>(initialOpenFromActive)
  ...
  const renderNavigationItems = useCallback(
    (items: TutorialNavigation[], parentPath: string = '') => {
      return items.map((item, index) => {
        ...
        if (item.list && item.list.length > 0) {
          return (
            <Accordion key={index} type="multiple" value={openAccordionItems} className="space-y-2">
              <AccordionItem value={item.path} className="border-none">
                <AccordionTrigger
                  ...
                  className={cn(
                    ...
                    isHydrated ? 'transition-colors duration-200' : 'transition-none',
                    {
                ...
                <AccordionContent disableAnimation={!isHydrated} className="pt-1 pb-0 pl-4">
