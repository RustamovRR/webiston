...
export const TerminalInput: React.FC<TerminalInputProps> = ({
  ...
  const renderAction = (action: TerminalInputAction, index: number) => {
    ...
    if (action.type === 'copy') {
      return <CopyButton key={index} text={action.text || value} disabled={action.disabled || !value || loading} />
    }
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div className="ml-2">
            <span className="text-lg font-medium text-zinc-900 dark:text-zinc-100">{title}</span>
      ...
      >
        ...
        {loading && (
          <div className="absolute inset-0 flex items-center justify-center bg-white/50 dark:bg-zinc-900/50">
            <div className="text-sm text-zinc-500 dark:text-zinc-400">{tCommon('processing')}</div>
          </div>
        )}
      </div>

      {/* Footer (optional) */}
      {showStats && stats.length > 0 && (statsPosition === 'footer' || statsPosition === 'both') && (
        <div
          className={cn(
