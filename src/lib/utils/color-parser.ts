...
export const parseColorInput = (
  ...
  const rgbaMatch = cleanInput.match(
    /^rgba\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*([0-9]*\.?[0-9]+)\s*\)$/
  )
  if (rgbaMatch) {
    return {
  ...
  const hslaMatch = cleanInput.match(
  ...
  const lchMatch = cleanInput.match(
    /^lch\s*\(\s*([0-9]*\.?[0-9]+)%?\s+([0-9]*\.?[0-9]+)\s+([0-9]*\.?[0-9]+)\s*\)$/
  )
  if (lchMatch) {
  ...
  const oklchMatch = cleanInput.match(
    /^oklch\s*\(\s*([0-9]*\.?[0-9]+)\s+([0-9]*\.?[0-9]+)\s+([0-9]*\.?[0-9]+)\s*\)$/
  )
  if (oklchMatch) {
