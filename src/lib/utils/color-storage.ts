...
export const addToColorHistory = (hex: string, name?: string): void => {
  ...
  try {
    ...
    const filtered = history.filter((item) => item.hex.toLowerCase() !== hex.toLowerCase())
...
export const addToColorFavorites = (hex: string, name: string, category?: string): void => {
  if (typeof window === 'undefined') return
  ...
  try {
    ...
    const exists = favorites.some((item) => item.hex.toLowerCase() === hex.toLowerCase())
    if (exists) return
...
export const removeFromColorFavorites = (hex: string): void => {
  ...
  try {
    const favorites = getColorFavorites()
    const filtered = favorites.filter((item) => item.hex.toLowerCase() !== hex.toLowerCase())
...
export const getFavoriteCategories = (): string[] => {
  const favorites = getColorFavorites()
  const categories = favorites.map((item) => item.category).filter((category): category is string => Boolean(category))
