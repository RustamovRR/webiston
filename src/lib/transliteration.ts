...
function transliteratePureUzbek(text: string): string {
  ...
  while (i < normalizedText.length) {
    ...
    if (char.toLowerCase() === "y") {
      ...
      if (
        (i === 0 || /[\s(\-"'«]/.test(normalizedText[i - 1])) &&
        nextChar.toLowerCase() === "e"
      ) {
    ...
    if (digraphs[twoChar]) {
      ...
      if (isUpperCase(originalCase[0]) && isUpperCase(originalCase[1]))
        cyrillicText += cyrillicChar.toUpperCase()
      else if (isUpperCase(originalCase[0]))
        cyrillicText +=
          cyrillicChar.charAt(0).toUpperCase() + cyrillicChar.slice(1)
    ...
    if (char === "'") {
      ...
      const next =
        i + 1 < normalizedText.length ? normalizedText[i + 1].toLowerCase() : ""
      const vowels = "aeiou"
    ...
    if (singleChars[lowerChar])
      cyrillicText += isUpperCase(char)
        ? singleChars[lowerChar].toUpperCase()
        : singleChars[lowerChar]
