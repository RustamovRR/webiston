## Qaysi API’ni qachon ishlatish kerak
...
Agar `renderToString` chaqiradigan server satrga aylantirish bilan band bo'lsa va 30 ta klient unga yangi so'rov yuborsa, bu yangi klientlar server hozirgi ishini tugatishini kutishlariga to'g'ri keladi. `renderToString` sinxron ishlagani sababli, u tugagunga qadar bloklanadi. Serverlar va klientlar o'rtasidagi birdan ko'pga munosabatda bloklanish, klientlar kerak bo'lgandan ko'ra uzoqroq kutishlarini anglatadi.
...
Shuni aytish kerakki, `renderTo*Stream` API’lari hozirda ustunroq ko'rinayotgan bo'lsa-da, hozirgi paytda ushbu API’lar atrofida "to'liq foydalanuvchi hikoyasi" mavjud emas. Hozir mavjud bo'lgan ko'plab uchinchi tomon kutubxonalari ular bilan ishlamaydi, ayniqsa ma'lumot olish yoki CSS kutubxonalari bilan bog'liq holda. Chunki ular serverda to'liq ishga tushirilgan bir martalik "to'liq ishlov berishni" talab qiladi, so'ngra ma'lumotlarni yaratadi va keyin o'sha ma'lumot bilan dasturni qayta ishlab, oqimdan foydalana boshlaydi. Ular dastur serverda hali to'liq yuklanmagan, ammo brauzerda qisman hidratsiya qilinishi kerak bo'lgan holatlarni qo'llab-quvvatlamaydi.
