...
Forma uchun harakat(action) qiymati `/cheese`ga teng va biz allaqachon *./routes/cheese.tsx* faylida joylashganmiz, shuning uchun forma xuddi shu route’ga yuborilishini taxmin qilishimiz mumkin. Ushbu route’ga `POST` metodi bilan kirilganida, forma yuborilganini bilamiz. Standart bo'yicha `GET` metodi bilan kirilganda esa, formani yuborish hali amalga oshirilmaganligini bilamiz va dastlabki foydalanuvchi interfeysini ko'rsatamiz.
