...
Endi ushbu misolni ishga tushirsak, `ExpensiveComponent`lar `count` uchun bir xil qiymat bilan renderlanadi, bu esa *tearing*’ning oldini oladi. `useSyncExternalStore` hook’i render vaqtida komponentning bir nechta nusxalari uchun state’ni bir xil bo'lishini ta'minlaydi.
