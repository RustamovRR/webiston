...
Shu bilan birga, bu yondashuv bilan bog'liq trade-off’lar ham mavjud. Svelte'ning kompilyatorga asoslangan tabiati virtual DOM’ga asoslangan freymvorklar taklif qiladigan ba'zi dinamik imkoniyatlarni, masalan, dinamik komponent turlarini ifodalashni biroz qiyinlashtiradi yoki murakkablashtiradi. Shuningdek, Svelte ekotizimi React, Vue va Angular’nikiga qaraganda kichikroq va yoshroq bo'lgani uchun, unda resurslar, kutubxonalar va hamjamiyat yechimlari kamroq bo'lishi mumkin.


### Runlar (Runes)
Runlar — bu Svelte kompilyatoriga ta'sir qiluvchi belgilar. Bugungi kunda Svelte `let`, `=`, `export` kalit so'zi va `$:` belgilaridan maxsus ma'nolar bilan foydalanadi, lekin runlar funksiyalar sintaksisi orqali shunga o'xshash va qo'shimcha imkoniyatlarni ham taqdim etadi.

Misol uchun, reaktiv holatni e'lon qilish uchun `$state` runidan foydalanish mumkin:
``` html copy {3}
<script>
// let count = 0;
 let count =$state(0);
...
Ushbu usul yordamida hisoblagich qiymatini saqlovchi (store) sifatida yaratib, uni bir nechta komponentlar o'rtasida osonlik bilan ulashish mumkin.
...
Bu ishlaydi, lekin biroz g'alati! Store API yanada murakkab narsalarni bajarishga uringanda, juda noqulay bo'lib qolishi mumkin. Runlar yordamida, bu jarayon ancha osonlashadi:
``` js copy {5, 10-11}
// import { writable } from 'svelte/store';
...
Bugungi kunda Svelte kompilyatsiya vaqtida reaktivlikdan foydalanadi. Bu, agar sizda `$:` yorlig'idan foydalanadigan ba'zi kod bo'lsa va u avtomatik ravishda qayta ishlanishi kerak bo'lsa, bu bog'lanishlar Svelte sizning komponentingizni kompilyatsiya qilganda aniqlanadi:
``` html copy
<script>
export let width;
export let height;

// kompilyator `area` ni qayta hisoblashini biladi
// `width` yoki `height` o'zgarganda...
$: area = width * height;

// ...va `area` ning qiymatini log qilishini biladi
// u o'zgarganda
$: console.log(area);
</script>
```

Bu yaxshi ishlaydi...garchi bu ishlamasa. Faraz qilaylik, biz kodni quyidagicha qayta tashkil qildik:
``` js copy
// @errors: 7006 2304
...
### Svelte 5 da yangi run
Svelte 5 `$derived `va `$effect` runlarini taqdim etadi, bu esa o'z ifodalari bog'lanishlarini baholanganida aniqlaydi:
``` html copy
<script>
