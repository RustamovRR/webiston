...
React state’ni boshqarish uchun mo'ljallangan ikkita hook’larni taklif etadi: `useState` va `useReducer`. Ikkalasi ham komponentda state’ni boshqarish uchun ishlatiladi. Farqi shundaki, `useState` birgina state’ni boshqarishga yaxshiroq mos kelsa, `useReducer` murakkabroq state’ni boshqarish uchun ishlatiladi. Keling, `useState` bilan komponentda state’ni boshqarish usulini ko'rib chiqaylik:
``` js copy
...
Ba’zilar bu usul `useState`ga qaraganda bir oz batafsilroq ekanligini aytadi va ko'pchilik bunga qo'shiladi, lekin bu abstraksiyalarni pastroq darajada ishlatishda odatiy hol: abstraksiya qanchalik past bo'lsa, kod shunchalik batafsilroq bo'ladi. Abstraksiyalar odatda murakkab logikani oddiyroq sintaksis bilan almashtirish uchun mo'ljallangan, bular ya'na *syntactic sugar* ham deb ataladi. 
...
Ushbu misolda biz ikki holatni test qilmoqdamiz: biri increment harakati reducer’ga uzatilganda, ikkinchisi esa noma'lum harakat uzatilganda.

Birinchi testda biz `count` qiymati 0 bo'lgan boshlang'ich state obyektini va increment harakati obyektini yaratmoqdamiz. Shundan keyin biz `count` qiymatining natijaviy state’da 1 ga oshirilishini kutmoqdamiz. Buning uchun `toEqual` matcher’idan foydalanib, kutilayotgan va asl state obyektlarini taqqoslaymiz.
...
#### 3. Voqeaga asoslangan modelni yaratadi
`useReducer` bu voqealarga asoslangan (event sourced) model bo'lib, bizning dasturimizda sodir bo'lgan voqealarni modellashtirish uchun ishlatilishi mumkin, bu voqealarni audit log turidagi ro'yxatga yozib borishimiz mumkin. Ushbu audit log dasturimizdagi voqealarni qayta ijro etish, xatolarni takrorlash yoki vaqt bo'yicha orqaga siljish orqali xatolarni tuzatish uchun ishlatilishi mumkin. Bundan tashqari, kuchli pattern’lar, masalan, bekor qilish/qaytarish, optimistik yangilanishlar va interfeysimizdagi umumiy foydalanuvchi harakatlarini kuzatish kabi imkoniyatlarni taqdim etadi.
