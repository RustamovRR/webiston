          ...
          <div className="ml-auto flex items-center gap-2">
            {ipInfo && (
              <>
                ...
                <span className="text-xs text-green-600 dark:text-green-400">{t('dataAvailable')}</span>
              </>
            )}
          </div>
        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4 rounded-lg border border-zinc-200/50 bg-zinc-100/30 p-4 dark:border-zinc-700/50 dark:bg-zinc-800/30">
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('ipAddress')}</div>
                    <div className="font-mono text-sm font-medium text-zinc-800 dark:text-zinc-200">{ipInfo.ip}</div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('ipType')}</div>
                    <div className="text-sm font-medium text-zinc-800 uppercase dark:text-zinc-200">{ipInfo.type}</div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('country')}</div>
                  ...
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('region')}</div>
                    <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
                      {ipInfo.region_name || t('unknown')}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('city')}</div>
                    <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
                      {ipInfo.city || t('unknown')}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('coordinates')}</div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4 rounded-lg border border-zinc-200/50 bg-zinc-100/30 p-4 dark:border-zinc-700/50 dark:bg-zinc-800/30">
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('isp')}</div>
                    <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
                      {ipInfo.connection.isp || t('unknown')}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('asn')}</div>
                    <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
                      {ipInfo.connection.asn ? `AS${ipInfo.connection.asn}` : t('unknown')}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('connectionType')}</div>
                    <div className="text-sm font-medium text-zinc-800 capitalize dark:text-zinc-200">
                      {ipInfo.connection.usage_type || t('unknown')}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('continent')}</div>
                    <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{ipInfo.continent_name}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4 rounded-lg border border-zinc-200/50 bg-zinc-100/30 p-4 dark:border-zinc-700/50 dark:bg-zinc-800/30">
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('timezone')}</div>
                    <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
                      {ipInfo.time_zone.id || t('unknown')}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('currency')}</div>
                    <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
                      {ipInfo.currency.code ? `${ipInfo.currency.name} (${ipInfo.currency.code})` : t('unknown')}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('callingCode')}</div>
                    <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
                      {ipInfo.location.calling_code ? `${ipInfo.location.calling_code}` : t('unknown')}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-500">{t('euMember')}</div>
                    <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
      ...
      {ipInfo && (
        <div className="rounded-xl border border-zinc-200 bg-white/80 backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/80">
          <div className="flex items-center justify-between border-b border-zinc-200 bg-zinc-50/50 px-4 py-3 dark:border-zinc-800 dark:bg-zinc-900/50">
            <h4 className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{t('jsonDataTitle')}</h4>
            <CopyButton text={JSON.stringify(ipInfo, null, 2)} />
          </div>
          <div className="p-4">
            <CodeHighlight code={JSON.stringify(ipInfo, null, 2)} language="json" className="max-h-80 overflow-auto" />
          </div>
        </div>
      )}
    </div>
