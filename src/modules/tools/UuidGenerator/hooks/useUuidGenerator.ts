...
export const useUuidGenerator = ({
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const newUuids: GeneratedUuid[] = Array.from(
        { length: count },
        (_, index) => {
  ...
  const validateUuid = useCallback((uuid: string): boolean => {
    ...
    return (
      uuidRegex.test(uuid) || compactRegex.test(uuid) || bracketRegex.test(uuid)
    )
  }, [])
  ...
  const getUuidInfo = useCallback((uuid: string) => {
    ...
    return {
      ...
      isValid: true,
      timestamp:
        versionDigit === "1" ? parseInt(cleanUuid.substring(0, 12), 16) : null
    }
  }, [])
