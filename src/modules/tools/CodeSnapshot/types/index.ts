...
/**
 * One theme, reduced to the five colours a swatch needs.
 *
 * Generated by `scripts/theme-palette.mjs`, which highlights a fixed snippet
 * with the same `codeToTokens` call this tool paints with and reads the colours
 * off the result — so a swatch cannot disagree with the picture it previews.
 * Never hand-edited: run `pnpm themes`.
 */
export interface ThemePalette {
  id: string
  /** The theme's own display name — a proper noun, never translated. */
  label: string
  isDark: boolean
  /** `editor.background`, the card behind the code. */
  bg: string
  fg: string
  comment: string
  keyword: string
  /** The colour of a plain declared name, e.g. `greet` in `const greet`. */
  identifier: string
  string: string
}
