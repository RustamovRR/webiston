  ...
  return (
    <div className="mb-6 rounded-xl border border-zinc-200 bg-white/80 p-6 backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/80">
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{t('readyTemplates')}</h3>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {presetTemplates.map((template, index) => (
            ...
            >
              <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{template.label}</div>
              <div className="mt-1 text-xs text-zinc-600 dark:text-zinc-400">{template.description}</div>
