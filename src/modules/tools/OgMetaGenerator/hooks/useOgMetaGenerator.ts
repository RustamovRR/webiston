...
const getPresetTemplates = (t: any) => [
  ...
  },
  {
    ...
    data: {
      title:
        "Webiston - O'zbekistondagi Eng Yaxshi Web Development Kompaniyasi",
      description:
  ...
  },
  {
    ...
    data: {
      title:
        "Web Development Conference 2024 - Toshkentda Eng Katta Tech Event",
      description:
...
export const useOgMetaGenerator = (
  { onSuccess, onError }: UseOgMetaGeneratorProps = {},
  t?: any
) => {
    ...
    try {
      ...
      if (metaData.type) {
        ...
        if (metaData.type === "article") {
          ogMetas.push(
            `<meta property="article:author" content="${metaData.twitterCreator || "Author"}" />`
          )
          ogMetas.push(
          ...
          ogMetas.push(
            `<meta property="book:author" content="${metaData.twitterCreator || "Author"}" />`
          )
      ...
      if (metaData.image) {
        ...
        ogMetas.push(
          `<meta property="og:image:alt" content="${metaData.title || "Image"}" />`
        )
      ...
      if (metaData.image) {
        ...
        twitterMetas.push(
          `<meta name="twitter:image:alt" content="${metaData.title || "Image"}" />`
        )
  ...
  const downloadMeta = useCallback(
    (format: "raw" | "formatted" = "raw") => {
      ...
      try {
        const filename =
          format === "formatted"
            ? `meta-tags-${Date.now()}.html`
            : `meta-tags-${Date.now()}.txt`
        const blob = new Blob([content], {
          type: format === "formatted" ? "text/html" : "text/plain"
        })
  ...
  const stats = useMemo(() => {
    ...
    const fieldsCompleted = Object.values(metaData).filter(
      (value) => value.trim() !== ""
    ).length
