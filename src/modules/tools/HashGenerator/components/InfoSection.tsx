    ...
    <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white/80 p-6 backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/80">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="font-semibold text-zinc-900 dark:text-zinc-100">{t('notes.title') || 'Muhim Eslatmalar'}</h3>
        </div>
        <ul className="space-y-1 text-sm text-zinc-600 dark:text-zinc-400">
          <li>• {t('notes.irreversible') || 'Hash funksiyalar qaytarilmas'}</li>
          <li>• {t('notes.deterministic') || 'Bir xil input → bir xil hash'}</li>
          <li>• {t('notes.avalanche') || "Kichik o'zgarish → butunlay boshqa hash"}</li>
          <li>• {t('notes.recommendation') || 'SHA256+ algoritmlarni ishlating'}</li>
        </ul>
      </div>
    </div>
  )
