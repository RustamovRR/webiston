...
export function AudioPreviewPanel({
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white/80 backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/80">
      ...
      <div className="p-6">
        <div className="mb-6">
          <AudioVisualizer audioStats={audioStats} isActive={isListening} width={500} height={200} className="w-full" />
        </div>
        ...
        {isListening && (
          ...
          >
            {!isRecording ? (
              <ShimmerButton onClick={onStartRecording} disabled={!isListening} className="bg-red-600 hover:bg-red-700">
        ...
        {isListening && audioInfo && (
          ...
          >
            <h4 className="mb-3 font-semibold text-zinc-800 dark:text-zinc-200">{t('audioInfo.title')}</h4>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-zinc-600 dark:text-zinc-400">{t('audioInfo.sampleRate')}:</span>
                <span className="ml-2 text-zinc-900 dark:text-zinc-100">{audioInfo.sampleRate} Hz</span>
              </div>
              <div>
                <span className="text-zinc-600 dark:text-zinc-400">{t('audioInfo.channels')}:</span>
                <span className="ml-2 text-zinc-900 dark:text-zinc-100">{audioInfo.channelCount}</span>
              </div>
              <div>
                <span className="text-zinc-600 dark:text-zinc-400">{t('audioInfo.echoCancellation')}:</span>
                <span
                  className={`ml-2 ${audioInfo.echoCancellation ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                >
                  {audioInfo.echoCancellation ? t('audioInfo.enabled') : t('audioInfo.disabled')}
                </span>
              </div>
              <div>
                <span className="text-zinc-600 dark:text-zinc-400">{t('audioInfo.noiseSuppression')}:</span>
                <span
                  className={`ml-2 ${audioInfo.noiseSuppression ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                >
                  {audioInfo.noiseSuppression ? t('audioInfo.enabled') : t('audioInfo.disabled')}
                </span>
              </div>
            </div>
          </motion.div>
