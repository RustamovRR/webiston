...
export const useMicrophoneTest = (options: UseMicrophoneTestOptions = {}) => {
  ...
  const getAudioDevices = useCallback(async () => {
    try {
      ...
      const audioInputs = devices
        ...
        .map((device) => {
          let label =
            device.label || `${t("deviceLabel")} ${device.deviceId.slice(0, 8)}`
  ...
  const getAudioQuality = useCallback(
    (level: number) => {
      if (level > 60)
        return { text: t("qualityExcellent"), color: "text-green-400" }
      if (level > 40) return { text: t("qualityGood"), color: "text-blue-400" }
      if (level > 20)
        return { text: t("qualityAverage"), color: "text-yellow-400" }
