...
const StrengthPanel: React.FC<StrengthPanelProps> = ({ password, passwordStrength, stats }) => {
    ...
    <div className="mt-6 rounded-xl border border-zinc-200 bg-white/80 p-6 backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/80">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rounded-lg border border-zinc-200 bg-zinc-100/30 p-4 text-center dark:border-zinc-700 dark:bg-zinc-800/30">
          <div className="text-2xl font-bold text-blue-500 dark:text-blue-400">{stats.characters}</div>
          <div className="text-xs text-zinc-500 dark:text-zinc-500">{t('characters')}</div>
        </div>
        <div className="rounded-lg border border-zinc-200 bg-zinc-100/30 p-4 text-center dark:border-zinc-700 dark:bg-zinc-800/30">
          <div className="text-2xl font-bold text-green-500 dark:text-green-400">{stats.unique}</div>
          <div className="text-xs text-zinc-500 dark:text-zinc-500">{t('unique')}</div>
        </div>
        <div className="rounded-lg border border-zinc-200 bg-zinc-100/30 p-4 text-center dark:border-zinc-700 dark:bg-zinc-800/30">
          <div className="text-2xl font-bold text-purple-500 dark:text-purple-400">{stats.entropy}</div>
          <div className="text-xs text-zinc-500 dark:text-zinc-500">{t('entropy')}</div>
        </div>
        <div className="rounded-lg border border-zinc-200 bg-zinc-100/30 p-4 text-center dark:border-zinc-700 dark:bg-zinc-800/30">
          <div className={cn('text-2xl font-bold', passwordStrength.color)}>{passwordStrength.level}/5</div>
          <div className="text-xs text-zinc-500 dark:text-zinc-500">{t('strength')}</div>
        </div>
      </div>
      ...
      <div className="mt-4">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-sm text-zinc-700 dark:text-zinc-300">{t('strengthLevel')}</span>
