/**
 * SEO Keywords for Latin-Cyrillic Converter
 * Organized by language and category
 */

// Yandex Metrics - eng ko'p qidirilgan
export const YANDEX_KEYWORDS = [
  "lotin krill",
  "lotin krill перевод online",
  "kirill lotin",
  "онлайн переводчик крилл-лотин",
  "krilcha hujjatni lotin tiliga ogirish",
  "lotin-kiril alifbosi tarjimon",
  "onlayn kiril-lotin",
  "лотин кирилл таржима",
  "kril lotin translate",
  "lotin krill tarjima",
  "lotin kirill",
  "lotin alifbosini tekshirish",
  "lotin to kirill converter",
  "kirill latin o-max",
  "крилча ва лотинча"
] as const

// Google Analytics - eng ko'p qidirilgan
export const GOOGLE_KEYWORDS = [
  "лотин кирилл",
  "лотин крил",
  "lotin kiril",
  "лотин-кирилл",
  "кирилл лотин",
  "latin to kiril",
  "крилл лотин"
] as const

// O'zbek tilida kengaytirilgan
export const UZBEK_KEYWORDS = [
  "lotin kirill online",
  "lotin kirill converter",
  "lotin kirill o'giruvchi",
  "lotin kirill o'zgartiruvchi",
  "kirill lotin online",
  "kirill lotin converter",
  "o'zbek lotin kirill",
  "o'zbek yozuvi o'girish",
  "o'zbek alifbosi converter",
  "lotin kirill bepul",
  "onlayn lotin kirill",
  "matn o'giruvchi",
  "o'zbek matn converter",
  "lotin kirill tool",
  "lotin kirill vosita",
  "o'zbek transliterator",
  "o'zbek script converter",
  "bepul lotin kirill",
  "tez lotin kirill",
  "aniq lotin kirill",
  "o'zbek harflar o'girish",
  "o'zbek alifbo o'girish",
  // Kirill yozuvida
  "лотин-кирилл",
  "кирилл лотин",
  "кирилл лотин онлайн",
  "лотин крил таржима программа",
  "лотин-кирилл онлайн",
  "лотин-кирилл таржимон",
  "лотин кирилл таржимон дастури",
  "кирилл лотин таржима"
] as const

// Ingliz tilida
export const ENGLISH_KEYWORDS = [
  "uzbek latin cyrillic",
  "uzbek text converter",
  "latin to cyrillic uzbek",
  "cyrillic to latin uzbek",
  "uzbek transliteration",
  "uzbek script converter",
  "uzbek alphabet converter",
  "free uzbek converter",
  "online uzbek converter",
  "uzbek transliterator",
  "uzbek language converter",
  "uzbek writing system",
  "uzbek script translator"
] as const

// Rus tilida
export const RUSSIAN_KEYWORDS = [
  "лотин кирилл переводчик",
  "кирилл лотин переводчик программа",
  "узбекский транслит",
  "узбекский транслитератор",
  "латиница кириллица узбекский",
  "кириллица латиница узбекский",
  "конвертер узбекского алфавита",
  "переводчик узбекского алфавита",
  "перевод с латиницы на кириллицу узбекский",
  "перевод с кириллицы на латиницу узбекский",
  "узбекская раскладка",
  "узбекская клавиатура",
  "кириллица в латиницу узбекский",
  "латиница в кириллицу узбекский",
  "онлайн-конвертер узбекский",
  "узбекский текст конвертер",
  "бесплатный узбекский конвертер",
  "узбекский алфавит онлайн",
  "узбекская письменность",
  "узбекский язык конвертер",
  "узбекский транслит онлайн",
  "узбекский переводчик алфавита",
  "конвертация узбекского текста",
  "узбекский текст латиница кириллица",
  "переключатель узбекского алфавита",
  "узбекская транслитерация",
  "узбекский алфавит переводчик",
  "узбекская раскладка онлайн",
  "узбекский конвертер текста бесплатно",
  "узбекский латинский кириллический",
  "узбекский алфавит конвертация",
  "узбекский текст транслит",
  "узбекская письменность конвертер"
] as const

// Fayl yuklash va eksport bilan bog'liq keywordlar
export const FILE_KEYWORDS = [
  // O'zbek
  "lotin kirill pdf converter",
  "lotin kirill docx o'girish",
  "lotin kirill txt converter",
  "lotin kirill fayl yuklash",
  "lotin kirill fayl o'girish",
  "lotin kirill hujjat converter",
  "pdf lotin kirill",
  "word lotin kirill",
  "txt lotin kirill",
  "fayl transliteratsiya",
  "hujjat o'giruvchi lotin kirill",
  "pdf dan kirill ga",
  "docx dan lotin ga",
  // Kirill
  "лотин кирилл pdf конвертер",
  "лотин кирилл docx таржима",
  "лотин кирилл файл юклаш",
  "pdf лотин кирилл",
  "word лотин кирилл",
  "ҳужжат ўгирувчи",
  // Rus
  "узбекский pdf конвертер латиница кириллица",
  "узбекский docx переводчик",
  "конвертер файлов узбекский",
  "pdf латиница кириллица узбекский",
  "word латиница кириллица узбекский",
  "загрузить файл узбекский конвертер",
  "скачать узбекский текст",
  // English
  "uzbek pdf latin cyrillic converter",
  "uzbek docx transliteration",
  "uzbek file converter",
  "upload file uzbek converter",
  "download uzbek text",
  "pdf to cyrillic uzbek",
  "word to latin uzbek"
] as const

// Long-tail keywords
export const LONG_TAIL_KEYWORDS = [
  "lotin kirill o'girish va ulardan foydalanish",
  "professional lotin kirill converter uzbek online",
  "узбекский транслитератор онлайн бесплатно латиница кириллица",
  "конвертер узбекского алфавита с латиницы на кириллицу онлайн",
  "webiston lotin kirill tools",
  "o'zbek lotin kirill converter professional",
  "uzbek latin cyrillic converter online free",
  "узбекский текст конвертер латиница кириллица бесплатно",
  "переводчик узбекского алфавита онлайн латиница кириллица",
  // Fayl bilan bog'liq long-tail
  "lotin kirill pdf docx txt converter online bepul",
  "o'zbek matn fayl yuklash va o'girish",
  "узбекский конвертер файлов pdf docx txt бесплатно",
  "uzbek file upload convert latin cyrillic free online"
] as const

// Barcha keywordlarni birlashtirish
/**
 * What actually ships in `<meta name="keywords">`.
 *
 * `ALL_KEYWORDS` is 144 entries — roughly 3.9 KB added to every render of the
 * site's most-visited page, for a tag Google has ignored since 2009 and Yandex
 * dropped years later. The three scripts people really search this tool in are
 * worth keeping; the long tail is not, and the duplicates in it definitely are
 * not. The full lists stay exported for reference and for anything that wants
 * to reason about search demand.
 */
export const PRIMARY_KEYWORDS = [
  "lotin kirill",
  "kirill lotin",
  "lotin kirill o'giruvchi",
  "lotin kirill tarjima",
  "lotin kirill alifbo jadvali",
  "kirilchadan lotinchaga",
  "lotinchadan kirillchaga",
  "лотин кирилл",
  "кирилл лотин",
  "лотин кирилл таржима",
  "переводчик кирилл латиница узбекский",
  "uzbek latin cyrillic converter",
  "uzbek transliteration",
  "pdf lotin kirill",
  "docx lotin kirill"
] as string[]

export const ALL_KEYWORDS = [
  ...YANDEX_KEYWORDS,
  ...GOOGLE_KEYWORDS,
  ...UZBEK_KEYWORDS,
  ...ENGLISH_KEYWORDS,
  ...RUSSIAN_KEYWORDS,
  ...FILE_KEYWORDS,
  ...LONG_TAIL_KEYWORDS
] as string[]
